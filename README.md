...
Плагин качает видео и гоняет ffmpeg — значит, ему нужна машина с интернетом, а не
облачная песочница. Отсюда таблица:

| Где вы работаете | Скилл | MCP-сервер |
|---|---|---|
| Claude Code в терминале | ✅ | не нужен |
| Вкладка Code в Claude Desktop | ✅ | не нужен |
| Cowork | ✅ | не нужен |
| Обычный чат Claude Desktop | ❌ | ✅ |
| claude.ai в браузере | ❌ | ❌ |

Обычный чат — и в браузере, и в приложении — выполняет код в облачной песочнице
Anthropic: `vkvideo.ru` там отдаёт `403 host_not_allowed`, yt-dlp не установлен, а
вашего диска и вашего ffmpeg не видно. Это не чинится настройками плагина. Для
этого случая в комплекте есть MCP-сервер: Desktop запускает его локально, и в
обычном чате появляется инструмент `analyze_vk_video` (см. раздел ниже).

Если вы всё же оказались в песочнице, скилл это заметит сам — он проверяет, лежит
ли рядом с ним `scripts/`, — и честно скажет, что площадка не та, вместо того чтобы
выдумывать содержание ролика.
...
Проверить, что всё встало, можно не спрашивая Клода — скрипт запускается сам по
себе и печатает JSON:

```bash
python skills/watch-vk-video/scripts/vk_fetch.py "https://vkvideo.ru/video-18403220_456239696" --no-transcribe --max-frames 4
```

`"ok": true` и пути к файлам — значит yt-dlp и ffmpeg на месте. `"error_code":
"no_ffmpeg"` — не хватает ffmpeg, в сообщении будет команда под вашу ОС.

## Обычный чат Claude Desktop

Скилл там не сработает, и это не лечится: обычный чат (и в приложении, и на
claude.ai) выполняет код в облачной песочнице Anthropic — без доступа к ВК, без
вашего ffmpeg и без вашего диска. Локально Desktop запускает только MCP-серверы,
поэтому в комплекте лежит `mcp_server.py` — второй вход в тот же пайплайн.

```bash
pip install mcp
```

Дальше в `claude_desktop_config.json` (Windows: `%APPDATA%\Claude\`,
...
**Правьте этот файл при полностью закрытом Desktop.** Запущенное приложение держит
конфиг в памяти и при выходе перезаписывает его своей версией — дописанный на ходу
`mcpServers` просто исчезнет. Порядок такой: выйти из Desktop целиком, включая
значок в трее, отредактировать файл, запустить приложение.

Путь до `python` лучше указать полный — Desktop не всегда наследует PATH. После
запуска в чате появляется инструмент `analyze_vk_video`: он качает ролик у вас на
машине и возвращает в чат название, транскрипт и кадры картинками. Кадров по
умолчанию 6, а не 40: каждая картинка стоит контекста.
